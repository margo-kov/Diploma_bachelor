...
\begin{figure}
	...
	% invalid reference
	% also, a "structure-based drug discovery" cycle -- from the figure it seems that structure is necessary, which is not always true
    \label{DrugCycle}
\end{figure}
...
The second major stage, drug development, is a process of bringing the potential 
medicine to market.
...
are applied, physical \acrlong{hts} (\acrshort{hts}) and computer-aided drug-design. % add a reference to a review
...
quick and cost-efficient.\\ % this statement needs a reference -- on some review, better

With respect to virtual screening, there are two different techniques: ligand-based 
and structure-based. % need a reference 
...
% would be nice to have a small section about Ultra-large library screening 
% and point out that people usually want to have immediately high-affinity compunds from such screening
% but it's up to you

\section{Small molecules and databases}

One of the significant questions is where to search candidates for lead discovery 
...
of a molecular structure with no information about anything but molecular topology, sometimes called a 1D-structure of a molecule. % ref: https://doi.org/10.1093/bioinformatics/bti314
...
The introduction of simple \acrshort{qsar} model had played a great role in understanding 
...
government institutions around the world. % can e.g. reference the Chemoinformatics book in Russian
...
numerical form -- molecular descriptors.
  $\text{In the Handbook of Molecular Descriptors}{\cite{Todeschini2007MethodsChemistry}}$, % btw, I think that references should not be superscripted
...
\hfill\break\\
 The ECFP rule for generating initial identifiers is derived from the properties 
...
atom environments considered.\\ % add an RDKit reference
...
and transitional freedom is considered;
  ...
  Scoring functions (\acrshort{sf}) must predict binding free-energy accurate enough, 
...
Usually, docking is implemented in an exhaustive style: the binding affinities of 
...
% describe a general idea behind the iterative algorithm: dock small part, train algorithm, dock only best-scoring ones
...
Another example of iterations usage is a $\text{paper}{\cite{Graff2020AcceleratingLearning}}$ 
